...
def json_to_csv(json_path: str, csv_path: str) -> None:
    ...
    if not (json_path[-5:] == ".json" and csv_path[-4:] == ".csv"):
        raise ValueError
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as filecsv:
        writer = csv.DictWriter(
            filecsv, fieldnames=data[0].keys(), extrasaction="raise"
        )
        writer.writeheader()
...
def csv_to_json(csv_path: str, json_path: str) -> None:
    ...
    with open(json_path, "w", newline="", encoding="utf-8") as filejson:
        json.dump(
            data, filejson, ensure_ascii=False, indent=2
        )  # список словарей, только ASCII символы, отступ уровня важности 2


a = csv_to_json(
    "././data/lab05/samples/csvPeople.csv", "././data/lab05/out/csvPeople.json"
)
